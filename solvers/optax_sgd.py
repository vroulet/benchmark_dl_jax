...
class Solver(BaseSolver):
    ...
    def run(self, n_iter):
        ...
        optimizer = optax.sgd(learning_rate=self.learning_rate, momentum=self.momentum, nesterov=self.nesterov)
        ...
        train_step_ = partial(train_step_template, obj_grad_fun=obj_grad_fun, optimizer=optimizer)
        train_step = jax.jit(train_step_)
        for _ in range(n_iter):
          for x, y in self.train_ds:
              params, batch_stats, opt_state = train_step(params, batch_stats, opt_state, x, y)
